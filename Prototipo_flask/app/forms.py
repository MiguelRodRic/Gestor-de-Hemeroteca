...
class Query(Form):
	...
	def classification(args,id, tag):
		...
		try:
			...
			for index in trainAux:
				...
				if X_ShuffleSplit[index][1] == clases['clase1']:
	...
	def explanation(args, id, tag):
		...
		for index in testAux:
			X_test_data.append(X_ShuffleSplit[index][0])
			if X_ShuffleSplit[index][1] == clases['clase2']:
				X_test_label.append(0)
			else:
				X_test_label.append(1)
